...
type Kind = "client" | "project" | "file" | "account" | "transaction";
type RecordItem = {
  id: string;
  kind: Kind;
  title: string;
...
type View = "Overview" | "Clients" | "Projects" | "Files" | "Accounts" | "Finances" | "Settings";
...
export default function AphurantaOS() {
  ...
  const transactions = useMemo(() => store.records.filter((item) => item.kind === "transaction"), [store.records]);
  ...
  const metrics = {
    ...
    income: transactions.filter((item) => item.transactionType === "income").reduce((sum, item) => sum + (Number(item.amount) || 0), 0),
    expenses: transactions.filter((item) => item.transactionType === "expense").reduce((sum, item) => sum + (Number(item.amount) || 0), 0),
  };
  ...
  const startCreate = (kind: Kind, projectId?: string, clientId?: string) => { setModalProjectId(projectId); setModalClientId(clientId); setModal(kind); };
  ...
  return (
    <main className="os-shell">
      ...
      <section className="os-work">
        ...
        {selected ? <RecordDetail item={selected} clients={clients} projects={projects} files={files} clientName={clientName} projectName={projectName} onBack={() => setSelectedId(null)} onSelect={openRecord} onPreview={setPreviewId} onAddFile={(projectId) => startCreate("file", projectId)} onAddProject={(clientId) => startCreate("project", undefined, clientId)} /> : <>
          ...
          {view === "Finances" ? <FinancePage transactions={transactions} projects={projects} clients={clients} create={() => startCreate("transaction")} remove={removeRecord} /> : null}
      ...
      {modal ? <CreateModal kind={modal} clients={clients} projects={projects} initialProjectId={modalProjectId} initialClientId={modalClientId} onClose={() => { setModal(null); setModalProjectId(undefined); setModalClientId(undefined); }} onSave={addRecord} /> : null}
...
function FinancePage({ transactions, projects, clients, create, remove }: { transactions: RecordItem[]; projects: RecordItem[]; clients: RecordItem[]; create: () => void; remove: (id: string) => void }) {
  const income = transactions.filter((item) => item.transactionType === "income").reduce((sum, item) => sum + (Number(item.amount) || 0), 0);
  const expenses = transactions.filter((item) => item.transactionType === "expense").reduce((sum, item) => sum + (Number(item.amount) || 0), 0);
  const net = income - expenses;
  const clientName = (id?: string) => clients.find((client) => client.id === id)?.title || "No client";
  const projectName = (id?: string) => projects.find((project) => project.id === id)?.title || "No project";
  return <div className="page"><div className="page-heading"><div><p className="eyebrow">Money in one place</p><h1>Finances</h1><p className="muted">Track income, expenses, and the project they belong to.</p></div><button className="primary" onClick={create}>＋ Add transaction</button></div><div className="finance-summary"><div className="finance-card positive"><small>Income</small><strong>{money(income)}</strong><span>Money received</span></div><div className="finance-card negative"><small>Expenses</small><strong>{money(expenses)}</strong><span>Money spent</span></div><div className="finance-card"><small>Net balance</small><strong>{money(net)}</strong><span>{net >= 0 ? "Positive balance" : "Needs attention"}</span></div></div><section className="panel list-panel finance-list"><div className="panel-heading"><div><h2>Transaction history</h2><p className="muted">Every saved movement, linked back to the workspace</p></div><span className="connection-count">{transactions.length}</span></div>{transactions.length ? <div className="rows">{[...transactions].sort((a, b) => String(b.date || b.createdAt).localeCompare(String(a.date || a.createdAt))).map((item) => <div className="data-row" key={item.id}><div className="row-main"><div className={`money-mark ${item.transactionType === "income" ? "income" : "expense"}`}>{item.transactionType === "income" ? "↑" : "↓"}</div><div><strong>{item.title}</strong><small>{item.category || "Uncategorised"} · {item.date || dateLabel(item.createdAt)} · {projectName(item.projectId)} · {clientName(item.clientId)}</small></div></div><div className="row-right"><strong className={item.transactionType === "income" ? "money-positive" : "money-negative"}>{item.transactionType === "income" ? "+" : "−"}{money(item.amount || 0)}</strong><button className="text-button danger" onClick={() => remove(item.id)}>Remove</button></div></div>)}</div> : <Empty title="No transactions yet" text="Add your first income or expense to start tracking." />}</section></div>;
}

function RecordDetail({ item, clients, projects, files, clientName, projectName, onBack, onSelect, onPreview, onAddFile, onAddProject }: { item: RecordItem; clients: RecordItem[]; projects: RecordItem[]; files: RecordItem[]; clientName: (id?: string) => string; projectName: (id?: string) => string; onBack: () => void; onSelect: (item: RecordItem) => void; onPreview: (id: string) => void; onAddFile: (projectId: string) => void; onAddProject: (clientId: string) => void }) {
...
function CreateModal({ kind, clients, projects, initialProjectId, initialClientId, onClose, onSave }: { kind: Kind; clients: RecordItem[]; projects: RecordItem[]; initialProjectId?: string; initialClientId?: string; onClose: () => void; onSave: (record: RecordItem) => void }) {
  const [values, setValues] = useState<Record<string, string>>({ status: "Planning", role: "Member", accountStatus: "Active", projectId: initialProjectId || "", clientId: initialClientId || "" });
